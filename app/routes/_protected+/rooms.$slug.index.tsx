...
import { type ReactElement, type Ref, type RefObject, useEffect, useRef, useState } from "react"
...
export default function RoomIndexRoute() {
	...
	return (
		<div className="isolate flex h-dvh flex-col gap-4 p-4">
			<JoinRoom />
			<TokenMap viewportRef={viewportRef} />
...
function TokenMap({ viewportRef }: { viewportRef: Ref<ViewportController> }) {
	const room = useRoom()
	const characters = useQuery(api.characters.list, { roomId: room._id })
...
function Toolbar({ viewportRef }: { viewportRef: RefObject<ViewportController> }) {
	const room = useRoom()
